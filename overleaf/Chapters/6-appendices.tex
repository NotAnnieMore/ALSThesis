...
\begin{table}[htbp]
\centering
\caption{Hyperparameter search spaces used in Optuna TPE optimisation.}
...
\begin{table}[htbp]
\centering
\caption{Best hyperparameters --- simple models (6-month horizon).}
...
\begin{table}[htbp]
\centering
\caption{Best hyperparameters --- gradient boosting models (6-month horizon).}
...
\begin{table}[htbp]
\centering
\caption{Feature block ablation (6-month, DEV set). PR-AUC with Optuna-tuned hyperparameters. Blocks are cumulative: each row adds new features to the previous.}
...
\begin{figure}[htbp]
  ...
  \caption{PR-AUC progression across cumulative feature blocks for four model families. FVC features (block~C) yield the largest marginal improvement.}
...
\begin{table}[htbp]
\centering
\caption{Held-out TEST set metrics (6-month horizon, $n{=}279$). PR-AUC and ROC-AUC are threshold-free; F$_1$ and F$_2$ use default $t{=}0.50$; Brier score measures calibration (lower is better).}
...
\begin{figure}[htbp]
  ...
  \caption{Precision--Recall curves on the held-out TEST set for all seven models.}
  \label{fig:app_pr}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Calibration plots (reliability diagrams) for all seven models on the TEST set.}
...
\begin{figure}[htbp]
  ...
  \caption{Confusion matrices at the default threshold ($t{=}0.50$) on the TEST set.}
  \label{fig:app_cm}
\end{figure}
...
To assess whether the observed performance differences among models are statistically significant, a Friedman test was applied to the per-fold PR-AUC scores from 5-fold GroupKFold cross-validation on the DEV set, followed by pairwise Wilcoxon signed-rank tests with Bonferroni correction.

\begin{table}[htbp]
  \centering
  \caption{Pairwise Wilcoxon signed-rank tests with Bonferroni correction (Friedman $\chi^2 = 12.43$, $p = 0.0531$).}
...
The Friedman test yields $\chi^2 = 12.43$ ($p = 0.053$), marginally above the significance threshold at $\alpha = 0.05$.
No pairwise comparison reaches significance after Bonferroni correction ($p_\text{Bonf.} = 1.000$ for all 21 pairs), confirming that the performance differences among the seven models are not statistically distinguishable given 5 cross-validation folds.
