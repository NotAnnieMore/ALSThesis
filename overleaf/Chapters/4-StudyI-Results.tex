...
FVC in litres is available for 56.3\% of the six-month cohort. Its availability shows no clear association with baseline \gls{ALSFRS-R}, age, or sex: the corresponding effect sizes are $d=0.027$, $d=0.031$, and $V=0.022$. The target slope comparison gives $p=0.050$ and $d=0.098$, which neither survives the Bonferroni threshold of 0.0045 nor indicates a large observed difference.

Study arm ($V=0.357$), \gls{BMI} ($d=0.468$), weight ($d=0.146$), and concomitant-medication count ($d=0.332$) remain the clearest differences after correction. This pattern is compatible with variation in measurement and recording practices across pooled trials, but it does not establish the reason for missing FVC. Median imputation is retained as a pragmatic preprocessing choice rather than as a consequence of a confirmed missing-data mechanism; \gls{MNAR} cannot be excluded. Appendix~\ref{app:fvc_missingness} reports all eleven comparisons.
...
The plausibility filter described in Section~\ref{sec:methods_bmi_correction} set 132 selected height measurements and 31 weights to missing before \gls{BMI} was recalculated. In the six-month cohort ($N=1{,}392$), the corrected BMI values range from 15.5 to 46.9, consistent with values reported for \gls{ALS} clinical-trial populations~\cite{Anani2023}. The horizon-specific datasets were regenerated from the cleaned vital-sign features before tuning, ablation, and all subsequent analyses. Appendix~\ref{app:bmi_correction} provides the full data-quality audit.
