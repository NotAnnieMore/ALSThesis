...
The 180 subjects without recorded age are concentrated in clinical trials in which age was not collected or was anonymised. This pattern suggests an administrative source of missingness, but it does not rule out differences between the affected trials and the remainder of the cohort. Age is therefore retained through fold-fitted median imputation rather than complete-case exclusion.
...
\noindent\textbf{FVC missingness.}
FVC in litres is available for 56.3\% of the six-month cohort. Availability shows no clear association with baseline \gls{ALSFRS-R}, age, sex, or the target slope after Bonferroni correction. Differences remain for study arm ($V=0.357$), \gls{BMI} ($d=0.468$), medication count ($d=0.332$), and weight ($d=0.146$), although the difference in weight is smaller than the other three. This pattern may reflect trial-specific measurement and recording practices, but it does not establish the missing-data mechanism; Appendix~\ref{app:fvc_missingness} reports all eleven comparisons.

\noindent\textbf{Missing-data sensitivity.}
With the tuned XGBoost configuration held fixed, primary median imputation reproduced the development \gls{PR-AUC} of $0.434\pm0.049$ and the TEST value of 0.456. Adding missingness indicators reduced mean development PR-AUC to $0.421\pm0.047$ and gave TEST PR-AUC 0.460. Five stochastic iterative imputations produced $0.426\pm0.044$ on development folds and 0.499 on TEST. The last value was obtained post hoc from the same held-out partition already used for the primary evaluation, so it does not replace the prespecified result or justify selecting a new preprocessing strategy. Its narrower implication is that performance did not deteriorate sharply under multivariable imputation. Appendix~\ref{app:study1_missing_sensitivity} reports the complete comparison.


\phantomsection\label{sec:results_bmi_fix}
\noindent\textbf{BMI correction.}
...
\noindent\textbf{Precision--recall and ROC curves.}
...
\noindent\textbf{Post-evaluation refit.}
...
\clearpage
\phantomsection\label{subsec:results_lime}
\noindent\textbf{LIME local explanations.}
...
\phantomsection\label{subsec:results_concordance}
...
\phantomsection\label{subsec:results_error_profile}
...
\phantomsection\label{subsec:results_dca}
...
\phantomsection\label{subsec:results_learning_curves}
