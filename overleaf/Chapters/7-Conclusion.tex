...
The shared contribution is therefore methodological rather than a single prognostic model. Across two outcomes, strong ranking performance did not determine the classifications produced at a particular operating point, and plausible feature attributions did not remove the need for separated validation or cautious clinical interpretation.
...
The MLP result exposed the limitation of selecting and reporting models through ranking metrics alone. It achieved cross-validated and test \gls{ROC-AUC} values of 0.915 and 0.918, yet detected only one of 35 Short Survivors at the default threshold. A sensitivity-constrained threshold selected from out-of-fold training predictions increased test sensitivity to 0.771 while retaining specificity of 0.883. The scores still ranked patients effectively; the failure occurred when the default boundary converted those scores into classes.
...
The clearest result appears in the gap between ranking and classification. In Study~I, lowering the operating threshold recovered most rapid progressors but generated many false alerts. In Study~II, the MLP ranked patients well while missing 34 of 35 Short Survivors at 0.5. Model assessment changed when the decision boundary and its errors were examined rather than inferred from AUC.
