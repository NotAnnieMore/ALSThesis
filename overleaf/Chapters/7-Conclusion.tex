...
Reproducibility depended on correcting imperfections as well as reporting them. Study~I kept final target construction, preprocessing, optimisation, and threshold selection outside the test partition. In Study~II, the implementation was revised so that MinMax scaling is fitted on the training partition and within each cross-validation fold. The transformed matrices and all held-out metrics remained unchanged because every ordinal predictor had the same range in the complete cohort, training partition, and fold-training samples.
