...
\begin{itemize}
  ...
  \item \textbf{FVC} ($n\!=\!3$): best \gls{FVC} in litres, best percentage of normal, and the recorded subject-normal value;
  ...
  \item \textbf{Labs} ($n\!=\!2$): creatinine and \gls{ALT}.
\end{itemize}
...
FVC in litres is available for 56.3\% of subjects, percentage of normal for 54.5\%, and the subject-normal value for 33.3\%. At least one of the three FVC features is present for 60.7\% of the cohort. Study arm is available for 91.1\%, while the operationally constructed riluzole indicator and medication count contain no missing values. Creatinine and \gls{ALT} coverage is 64.7\% and 69.0\%, respectively. These differences are reported per feature in Table~\ref{tab:feature_summary} and motivate the fold-specific imputation described later.
...
\subsection{Exploratory Review of Treatment Variables}
...
A post-hoc sensitivity analysis retained the selected unbalanced XGBoost hyperparameters and compared three numeric-data strategies: the primary median imputation, median imputation with missingness indicators, and five stochastic iterative imputations. For the third strategy, a separate model was fitted to each imputed training set and the five validation or test probabilities were averaged. Numeric imputers were fitted within each cross-validation training fold and on \gls{DEV} before application to \gls{TEST}; categorical processing remained unchanged. This analysis tests whether the main ranking estimates depend on one simple imputation rule. It does not identify the missing-data mechanism or make an \gls{MNAR} process ignorable~\cite{white2011mice,sterne2009multiple}.
...
\gls{PR-AUC}, implemented as average precision, is the primary optimisation and model-ranking metric. \gls{ROC-AUC} is reported as a secondary ranking measure, while $F_1$ and $F_2$ at the default threshold of 0.5 are retained for descriptive comparison. Spearman correlations across the 13 configurations are used to compare the rankings produced by these metrics.

The operating threshold is selected only after model selection, using out-of-fold predictions from the development set as described in Section~\ref{sec:methods_threshold}. Synthetic over- or under-sampling is not used in Study~I; the imbalance analysis is restricted to the native weighting options above so that every model is trained on the observed development records.
...
\phantomsection\label{subsec:methods_concordance}
\noindent\textbf{SHAP--LIME concordance.}
...
\phantomsection\label{subsec:methods_lr_coefs}
\noindent\textbf{Logistic Regression coefficients.}
...
\noindent\textbf{Subgroup performance analysis.}
...
\noindent\textbf{Decision curve analysis.}
...
\noindent\textbf{Calibration analysis.}
...
\noindent\textbf{Partial dependence plots.}
