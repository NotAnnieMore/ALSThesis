...
PRO-ACT is selected because no comparable public ALS dataset approaches its scale ($>$10{,}000 subjects) or breadth of longitudinal clinical variables; smaller alternatives (e.g.\ single-centre registries) lack the statistical power required for robust model development and cross-validation.
...
The 80/20 ratio follows standard ML practice for moderate-sized datasets: 80\% provides sufficient data for 5-fold cross-validation within the DEV set, while 20\% yields a TEST set large enough ($N\!=\!279$ for 6 months) for stable bootstrap confidence intervals.
...
Both horizons are explored; the 6-month window is ultimately preferred because it yields more data points per subject for slope estimation (median $n_{\text{points}} = 7$), produces more reliable progression labels, and aligns with the clinical follow-up cadence recommended for ALS patients~\cite{Pancotti2022}.
...
We use the total ALSFRS-R score rather than individual sub-items or survival endpoints because it captures overall functional decline in a single continuous measure, is the most widely adopted outcome in ALS clinical trials, and permits direct comparison with prior prognostic studies~\cite{Pancotti2022, Anani2023}.
...
Deep learning architectures (e.g.\ DNNs, CNNs) are excluded because the cohort size ($N\!=\!1{,}392$ for 6 months) is insufficient to train deep models without severe overfitting, and because black-box neural networks conflict with the explainability requirement that is central to this work.
...
\begin{enumerate}
  \item \textbf{Dataset lookup:} a subject ID from the held-out TEST set;
  \item \textbf{JSON file:} a key--value file with baseline feature values, allowing external or synthetic patients;
  \item \textbf{Demo mode:} a randomly selected TEST-set patient for quick demonstration.
\end{enumerate}

Each report comprises three sections:
\begin{itemize}
  \item \textbf{Risk Assessment.} The predicted probability of rapid progression $P(\text{rapid})$ is displayed alongside a three-tier risk classification (LOW: $P < 0.21$; MODERATE: $0.21 \leq P < 0.50$; HIGH: $P \geq 0.50$).
  Binary classifications at both the $F_1$-optimal and $F_2$-optimal thresholds ($t = 0.21$) are provided.
  When the true label is available (dataset patients), it is shown for verification.
  \item \textbf{Top-5 Contributing Features (SHAP).} A local TreeSHAP explanation lists the five features with the largest absolute Shapley values, together with the feature value, SHAP magnitude, and direction of influence.
  ...
  Thirteen features have predefined rules comparing the patient's value against population percentiles (25th or 75th) computed from the DEV set ($N = 1{,}113$).
  For example, an ALSFRS-R total below the 25th percentile (35~points) triggers a recommendation for monthly functional reassessment.
\end{itemize}
...
Each continuous variable is sampled from a truncated normal distribution calibrated on the full PRO-ACT dataset ($N = 1{,}392$), matching the observed mean, standard deviation, and range.
Categorical variables (sex, ethnicity, race, riluzole use) are sampled with the observed population frequencies.
Logical correlations are preserved: FVC in litres is conditioned on sex (males $\mu = 3.9$~L, females $\mu = 2.8$~L), and BMI is derived from the sampled weight and height rather than drawn independently.
