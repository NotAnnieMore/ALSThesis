...
To describe which participants the model misclassifies, all \gls{TEST}-set subjects are assigned to a confusion-matrix quadrant (\gls{TP}, \gls{FP}, \gls{FN}, or \gls{TN}) at the selected threshold and stratified by four clinical variables:
\begin{itemize}
...
\end{itemize}
The age and \gls{FVC} boundaries are quartile-derived cut-points among participants with observed values.
Participants with missing age or \gls{FVC} are retained in the error profile as a separate \textit{Missing} group rather than being omitted from the figures.
These variables represent demographics (age and sex), overall functional status (\gls{ALSFRS-R}), and respiratory function (\gls{FVC}).
For each group, absolute counts and within-group proportions are reported for all four quadrants, with particular attention to false negatives because missed rapid progressors are the principal concern at a screening-oriented operating point.

\subsection{Subgroup Performance Analysis}
\label{subsec:methods_subgroup}
To explore whether model performance varies across clinical subgroups, we compute \gls{PR-AUC}, recall, precision, and $F_1$ separately for each stratum of the four variables above.
...
Missing age and \gls{FVC} values are retained as separate groups when they meet these criteria.
The overall \gls{TEST}-set \gls{PR-AUC} and recall are included in the figures as descriptive reference lines.
Because subgroup sizes and outcome prevalences differ, and no multiplicity-adjusted hypothesis tests or subgroup-specific confidence intervals are used, this analysis is exploratory and does not establish fairness or equivalence.
In particular, absolute \gls{PR-AUC} values are interpreted alongside the rapid-progressor prevalence in each subgroup.
...
A model has a higher estimated net benefit at thresholds where its curve exceeds both references.
The analysis is evaluated over $t \in [0.01,\, 0.60]$ in increments of 0.01.
No participant has a predicted probability above 0.58, so higher thresholds are equivalent to the treat-none strategy and add no information.
Because the analysis uses the same finite TEST sample as the performance evaluation and does not include confidence intervals, isolated threshold points are interpreted cautiously; emphasis is placed on continuous intervals of higher net benefit.
...
To explore sensitivity to training-set size, the selected XGBoost configuration is trained on progressively larger subsets of DEV (20\%, 30\%, \ldots, 100\%) and evaluated on the fixed \gls{TEST} set.
DEV contains one row per participant, so subsampling is performed directly at participant level.
Each fraction below 100\% is repeated 20 times using stratified random samples, preserving the rapid-progressor proportion at approximately 30\%; the full DEV fit is deterministic and is therefore run once.
The figure reports the mean \gls{PR-AUC} and \gls{ROC-AUC} across repetitions, with error bars representing one standard deviation rather than confidence intervals.
Because the same TEST set is evaluated repeatedly, this is treated as a post-evaluation sensitivity analysis and is not used for model or hyperparameter selection.
The curve can indicate whether performance continues to change with sample size, but it cannot by itself establish a fixed performance ceiling or predict the benefit of collecting additional data.
...
We compute \glspl{PDP} for the three features with the highest global \gls{SHAP} importance: \gls{ALSFRS-R} total, \gls{FVC} in litres, and age.
For each feature, the grid spans the 5th to 95th percentile of its observed values in TEST; missing values are excluded when defining the grid and rug plot but remain median-imputed within the model pipeline for prediction.
A rug plot shows the observed TEST values and therefore makes sparse regions and missingness easier to recognise.
PDPs average over the joint distribution of all remaining variables and can evaluate feature combinations that are uncommon or clinically implausible, particularly when predictors are correlated.
They are consequently interpreted as descriptive summaries of the fitted model, not as causal dose--response relationships or individual-level effects.

\section{Patient Risk Report Generator}
\label{sec:methods_patient_report}

As a post-evaluation software demonstrator, we developed a \textit{patient risk report generator} that presents the output of the persisted XGBoost pipeline for one patient record.
The report uses the model refitted on the complete eligible cohort after held-out evaluation (Section~\ref{sec:methods_final_model}); it must therefore not be used to estimate predictive performance.
The tool loads the pipeline and its associated metadata (feature list, slope cutoff, and decision threshold) and accepts data from three sources:
\begin{enumerate}
  \item \textbf{Dataset lookup:} a subject ID from the processed cohort;
...
\begin{itemize}
  \item \textbf{Model output.} The estimated probability of rapid progression, $P(\text{rapid})$, is shown together with the binary classification obtained at the preselected operating threshold ($t=0.21$).
  For cohort records, the retrospectively derived outcome is also displayed, but only as contextual information because those records contributed to the final refit.
  ...
  Contributions from one-hot encoded columns are aggregated under the corresponding source variable.
  These values explain the fitted model for that record; they do not identify causal effects.
  \item \textbf{Contextual review flags.} For 13 variables, descriptive rules identify whether a top-five contributor lies beyond a predefined 25th- or 75th-percentile cutoff derived from \gls{DEV}.
  The flags provide distributional context only.
  They are not clinical reference ranges, alerts, or recommendations for follow-up or treatment.
\end{itemize}

The generated text includes an explicit research-use disclaimer.
Neither the predicted probability nor the displayed threshold and review flags have undergone prospective clinical validation.

\subsection{Synthetic Patient Generator}
\label{subsec:methods_random_patient}
To test the report interface without selecting a named cohort record, a companion script generates fictitious profiles containing all 35 baseline features.
Continuous variables are sampled from bounded distributions calibrated to approximate observed summaries in the full \gls{PRO-ACT} cohort ($N=1{,}392$), while categorical variables use approximate observed frequencies.
Two dependencies are imposed explicitly: \gls{BMI} is calculated from sampled weight and height, and \gls{FVC} in litres is conditioned on sex.
The remaining variables are sampled separately, so the generator does not reproduce the full joint distribution of the clinical data.
These profiles are suitable for testing data ingestion, prediction, and report formatting, but not for clinical simulation, privacy assessment, or model validation.
