"""
Optuna tuning for XGBoost on 6-month horizon (ALS rapid progression).
...
Outputs:
- 04_outputs/tables/optuna_xgb_6m_trials.csv
...
def make_xy(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, pd.Series]:
    ...
    if not feat_cols:
        raise ValueError("No feature columns available after dropping ID/target columns.")
...
def main():
    ...
    # Ajusta aqui conforme quiseres
    n_trials = 60
    timeout_s = None  # ex.: 3600

    print(f"[INFO] Starting Optuna for XGBoost (6m) | N={len(X)} | trials={n_trials}")
    ...
    trials_path = os.path.join(out_dir, "optuna_xgb_6m_trials.csv")
    best_path = os.path.join(out_dir, "optuna_xgb_6m_best_params.json")
